...
async def create_payment(payment_in: PaymentCreate, repo: PaymentRepoDep, _admin: AdminUser) -> PaymentRead:
    payment = await payment_service.create_payment(repo, data=payment_in.model_dump())
...
async def patch_payment(
    payment_id: UUID, payment_in: PaymentUpdate, repo: PaymentRepoDep, _admin: AdminUser
) -> PaymentRead:
...
async def delete_payment(payment_id: UUID, repo: PaymentRepoDep, _admin: AdminUser) -> Response:
    await payment_service.delete_payment(repo, payment_id=payment_id)
