...
@pytest.mark.smoke
def test_invoice_and_payment_default_enums() -> None:
    assert Invoice.__table__.c.status.nullable is False
    assert str(Invoice.__table__.c.status.default.arg) == InvoiceStatus.PENDING
    assert Payment.__table__.c.kind.nullable is False
    assert str(Payment.__table__.c.kind.default.arg) == PaymentKind.PAYMENT
